...
export async function POST(
  ...
  try {
    ...
    const memberRole = roleParam === 'admin' ? 'admin' : 'member';

    // Run migrations before attempting inserts
    await runMigrations();
    ...
    const { name, email: memberEmail, password } = body as { name: string; email: string; password: string };
    ...
    const userResult = await query(
      ...
      [name.trim(), memberEmail.toLowerCase(), passwordHash, invite.user_id]
    );

    const userId = userResult.rows[0].id;

    // Link this user as a team member of the inviting user with specified role
    await query(
      `INSERT INTO team_members (owner_id, member_id, role)
       VALUES ($1, $2, $3)
       ON CONFLICT (owner_id, member_id) DO NOTHING`,
      [invite.user_id, userId.toString(), memberRole]
    );
